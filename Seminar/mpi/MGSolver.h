...
public:
   ...
   void solveFMG( Grid& x, const Grid& f, size_t pre, size_t post ,size_t rank);
   // Utility functions
...
private:
   ...
   void calcResidual( Grid & x, const Grid & f, Grid & r );
   void restrict( Grid & r, Grid & f );
...
void MGSolver::solveFMG(Grid& x, const Grid& f, size_t pre, size_t post,size_t rank)
{
       real res_cur;
      worker = rank;
...
{
    real send_buffer[x.ysize()],recv_buffer[x.ysize()];
    MPI_Request request;
    MPI_Status status;
  
    for (size_t i=1;i<x.ysize();++i)
      send_buffer[i] = x(i,x.xsize()-3);
    MPI_Isend (send_buffer, x.ysize(), MPI_DOUBLE, (worker+1)%2, 77, MPI_COMM_WORLD, &request);
    MPI_Wait (&request, &status);

    MPI_Irecv (recv_buffer, x.ysize(), MPI_DOUBLE, (worker+1)%2, 77, MPI_COMM_WORLD,&request);
    MPI_Wait (&request, &status);
    for (size_t i = 1;i<x.ysize();++i)
      x(i,x.xsize()-1) = recv_buffer[i];
  ...
  for( size_t i = 1; i < x.ysize(); i++ ){
	...
	x( i, j ) = x.hsize()*x.hsize() * A_.C * f( i, j )
      ...
      else if ((worker == 0)&&(j<(x.xsize()-1)/2))
	x( i, j ) = x.hsize()*x.hsize() * A_.C * f( i, j )
		+ ( A_.N * x( i - 1, j ) + A_.S * x( i - 1, j ) + A_.W * x( i, j - 1 ) + A_.E * x( i, j + 1 ) );
      }
  }
...
void MGSolver::calcResidual( Grid& x, const Grid& f, Grid& r )
{
    real send_buffer[x.ysize()],recv_buffer[x.ysize()];
    MPI_Request request;
    MPI_Status status;
...
void MGSolver::restrict( Grid & r, Grid & f )
{
    real send_buffer[r.ysize()],recv_buffer[r.ysize()];
    MPI_Request request;
    MPI_Status status;
  
    for (size_t i=1;i<r.ysize();++i)
      send_buffer[i] = r(i,r.xsize()-3);
    MPI_Isend (send_buffer, r.ysize(), MPI_DOUBLE, (worker+1)%2, 77, MPI_COMM_WORLD, &request);
    MPI_Wait (&request, &status);

    MPI_Irecv (recv_buffer, r.ysize(), MPI_DOUBLE, (worker+1)%2, 77, MPI_COMM_WORLD,&request);
    MPI_Wait (&request, &status);
    for (size_t i = 1;i<r.ysize();++i)
      r(i,r.xsize()-1) = recv_buffer[i];
   ...
   for( size_t i = 1; i < f.ysize() ; ++i ){
	...
	else if ((worker == 0)&&(j<(f.xsize()-1)/2))
            f( i, j ) = I_.C * r( i * 2, j * 2 ) + I_.N * r( i * 2 - 1, j * 2 ) + I_.S * r( i * 2 - 1, j * 2 ) + I_.W * r( i * 2, j * 2 - 1 )
                        + I_.E * r( i * 2, j * 2 + 1 ) + I_.NW * r( i * 2 - 1, j * 2 - 1 ) + I_.NE * r( i * 2 - 1, j * 2 + 1 )
                        + I_.SW * r( i * 2 - 1, j * 2 - 1 ) + I_.SE * r( i * 2 - 1, j * 2 + 1 );
...
void MGSolver::prolongateAndCorrect( const Grid& e, Grid& x )
{
   ...
   for( size_t i = 1; i < e.ysize() ; ++i ){
	...
	}else if ((worker == 0)&&(j<(e.xsize()-1)/2)){
	    x( i * 2, j * 2 ) += J_.C * e( i, j );
            x( i * 2 - 1, j * 2 ) += J_.S * e( i, j );
            x( i * 2, j * 2 - 1 ) += J_.W * e( i, j );
            x( i * 2, j * 2 + 1 ) += J_.E * e( i, j );
